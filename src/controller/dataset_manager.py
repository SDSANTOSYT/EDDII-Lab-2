...
def airports_adjacency_list() -> dict:
    with open('src/flights_final.csv',"r",encoding="utf-8") as dataset:
        ...
        airport_connections = {airport: list(set([connection[6] for connection in data if connection[0] == airport] + [connection[0] for connection in data if connection[6] == airport])) for airport in airport_node_list().keys()}
